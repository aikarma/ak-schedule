...
export function Launcher(appConfig: AppConfig) {
    ...
    (async () => {
        ...
        for (let jobConfig of jobConfigs) {
            ...
            if (paramtypes == null) {
                console.error(new Error(`emitDecoratorMetadata is required, enable it in tsconfig.json:
                {
                    "compilerOptions": {
                ...
                for (let i = 0, len = paramtypes.length; i < len; i++) {
                    ...
                    if (paramtype == Job || Job.isPrototypeOf(paramtype)) {
                        ...
                        if (workerFactory) {
                            ...
                            console.error(new Error("parameters of " + target.name + "." + method + " is invalid, type of parameter " + paramnames[i] + " should be a class, not an interface."));
                            process.exit(-1);
                        }
                        else {
                            console.error(new Error("parameters of " + target.name + "." + method + " is invalid, WorkerFactory of parameter " + paramnames[i] + " is not found."));
                            process.exit(-1);
                        }
                    }
                }
            ...
            if (hasError) {
                console.error(new Error("parameters of " + target.name + "." + method + " is invalid, at most one (job: Job) parameter and at most one (worker: WorkerType) are required"));
                process.exit(-1);
            }
        }
        // 初始化 target
        ...
        // console.log("init db(" + appConfig.dbUrl + ") successfully");
